...
export async function transferAssets(keypair, type, amount, toPublicKey) {
  await this._getConnection();
  const assets = [];
  const unSpent = await this.conn.listOutputs(keypair.publicKey, false);
  if (!unSpent || !unSpent.length) {
...
export async function transferDivisibleAsset(tx: any, fromKeyPair, toPublicKeysAmounts, metadata) {
    ...
    for (const entry of toPublicKeysAmounts) {
        const output = driver.Transaction.makeOutput(driver.Transaction.makeEd25519Condition(entry.publicKey), entry.amount.toString());
